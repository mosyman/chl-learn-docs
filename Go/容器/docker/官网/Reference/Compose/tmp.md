...
Note that support for `uid`, `gid`, and `mode` attributes are not implemented in Docker Compose when the source of the secret is a [`file`](/reference/compose-file/services/secrets/). This is because bind-mounts used under the hood don't allow uid remapping.
