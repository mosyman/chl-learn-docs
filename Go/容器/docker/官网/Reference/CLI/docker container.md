...
- [docker container run](#docker-container-run别名-docker-run)
  - [`-i/--interactive`、`--init`、`-t/--tty` ](#-i--interactive--init-t--tty-)
  - [`--restart`](#--restart-参数容器重启策略的用法)
  - [`--rm`](#--rm)
- [docker container exec](#docker-container-exec别名-docker-exec)
- [docker container inspect](#docker-container-inspect)
- [docker container start](#docker-container-start别名-docker-start)
- [docker container stop](#docker-container-stop别名-docker-stop)


## docker container run（别名 `docker run`）
...
## docker container exec（别名 `docker exec`）

### 一、命令核心作用
`docker exec` 是 Docker 中最常用的命令之一，核心作用是：**在已经运行的容器内执行一条新的命令**。

这个命令的关键特点：
1. 仅对**运行中的容器**有效（容器状态为 `running`，暂停/停止的容器无法执行）；
2. 执行的命令是“临时”的，仅在容器当前运行周期内生效，容器重启后不会自动重新执行；
3. 命令默认在容器的**默认工作目录**（比如容器启动时指定的 `WORKDIR`）执行；
4. 不支持直接执行“链式命令”（如 `echo a && echo b`），必须通过 `sh -c` 包裹（下文会举例）。

### 二、核心参数详解（Options）
| 参数 | 简写 | 作用 | 实用场景 | 关键说明 |
|------|------|------|----------|----------|
| `--detach` | `-d` | 后台执行命令（分离模式） | 执行无需交互的耗时命令（如创建文件、备份数据） | 命令在后台运行，执行后立即返回，不阻塞终端 |
| `--detach-keys` | - | 自定义“脱离容器”的快捷键 | 替换默认的 `Ctrl+P,Ctrl+Q` 脱离组合键 | 新手极少用到，主要用于避免快捷键冲突 |
| `--env` | `-e` | 为执行的命令设置环境变量 | 临时指定环境变量执行命令（如数据库连接参数） | 仅对本次 `exec` 执行的命令生效，不影响容器全局环境 |
| `--env-file` | - | 从文件读取环境变量 | 批量设置多个环境变量 | 文件格式和 `docker run --env-file` 一致（每行 `KEY=VALUE`） |
| `--interactive` | `-i` | 保持标准输入（STDIN）打开 | 交互式命令（如进入容器shell） | 单独 `-i` 可接收输入，但无终端交互；通常和 `-t` 组合用 |
| `--privileged` | - | 给执行的命令赋予“扩展权限” | 需要容器内执行高权限操作（如修改系统配置） | 相当于给命令赋予宿主机的 root 权限，慎用（安全风险） |
| `--tty` | `-t` | 分配伪终端（PTY） | 交互式终端操作（如 `sh`/`bash`） | 模拟真实终端环境，让命令行操作和本地一致；通常和 `-i` 组合为 `-it` |
| `--user` | `-u` | 指定执行命令的用户/UID | 非 root 权限执行命令（安全） | 格式：`用户名`/`UID` 或 `用户名:组名`/`UID:GID`（如 `-u 1000`、`-u appuser:appgroup`） |
| `--workdir` | `-w` | 指定命令执行的工作目录 | 切换目录执行命令（如在 `/app` 目录下执行脚本） | 仅本次命令生效，不改变容器默认工作目录 |

### 三、典型使用场景与示例
#### 场景1：最常用 - 进入运行中的容器（交互式终端）
这是 `docker exec` 最核心的使用场景，通过 `-it` 组合进入容器的 shell 环境：
```bash
# 进入名为 mycontainer 的容器，启动 sh 终端（Alpine 系统用 sh，Ubuntu/CentOS 用 bash）
docker exec -it mycontainer sh

# 或指定用户（非 root）进入
docker exec -it -u 1000 mycontainer bash
```
- 执行后会进入容器内部的命令行，可像操作本地服务器一样执行命令（如 `ls`、`cat`、`ps` 等）；
- 退出容器：执行 `exit` 或按 `Ctrl+D`（不会停止容器，只是退出当前终端）。

#### 场景2：后台执行无交互命令（-d 参数）
无需交互的命令（如创建文件、执行脚本），用 `-d` 后台执行：
```bash
# 在容器后台创建 /tmp/test.txt 文件
docker exec -d mycontainer touch /tmp/test.txt

# 后台执行备份脚本（假设容器内有 /app/backup.sh）
docker exec -d mycontainer sh -c "/app/backup.sh > /tmp/backup.log 2>&1"
```

#### 场景3：指定环境变量执行命令（-e 参数）
临时设置环境变量，仅对本次命令生效：
```bash
# 在容器内执行 env 命令，查看临时设置的 VAR_A 和 VAR_B
docker exec -e VAR_A=100 -e VAR_B=200 mycontainer env
...
#### 场景4：指定工作目录执行命令（-w 参数）
切换到指定目录再执行命令，避免手动 `cd`：
```bash
# 默认工作目录执行 pwd
docker exec mycontainer pwd  # 输出：/

# 指定 /root 目录执行 pwd
docker exec -w /root mycontainer pwd  # 输出：/root

# 在 /app 目录下执行脚本
docker exec -w /app mycontainer sh -c "./start.sh"
```

#### 场景5：执行链式/复杂命令（必须用 sh -c 包裹）
`docker exec` 不支持直接执行 `&&`/`||` 等链式命令，必须通过 `sh -c` 包裹整个命令串：
```bash
# ✅ 正确：用 sh -c 执行链式命令
docker exec -it mycontainer sh -c "echo 'hello' && echo 'world' > /tmp/hello.txt"

# ❌ 错误：直接执行链式命令会失败
docker exec -it mycontainer "echo 'hello' && echo 'world'"
```

#### 场景6：高权限执行命令（--privileged 参数）
需要修改容器内系统级配置（如挂载设备、修改内核参数）时，用 `--privileged` 提升权限：
```bash
# 高权限进入容器（谨慎使用）
docker exec -it --privileged mycontainer bash
```

### 四、注意事项与常见问题
1. **仅对运行中的容器有效**：
   暂停的容器（`docker pause`）执行 `exec` 会报错，需先执行 `docker unpause` 恢复运行；
   停止的容器（`docker stop`）需先 `docker start` 启动后才能执行。

2. **命令必须是可执行的**：
   容器内必须存在要执行的命令（如 `bash`、`sh`、`ls`），否则会报错 `exec: "bash": executable file not found in $PATH`（比如 Alpine 系统默认没有 `bash`，只能用 `sh`）。

3. **权限问题**：
   默认以 `root` 用户执行命令，如需限制权限，用 `-u` 指定普通用户（需容器内已创建该用户）。

4. **退出容器不影响容器运行**：
   用 `exit` 退出 `exec` 进入的终端，容器仍会继续运行（区别于 `docker run -it`，退出后容器会停止）。

### 总结
1. `docker exec`（全称 `docker container exec`）是在**运行中的容器**内执行命令的核心命令，最常用组合是 `-it` 进入交互式终端；
2. 核心参数：`-it` 交互式终端、`-d` 后台执行、`-e` 临时环境变量、`-w` 指定工作目录、`-u` 指定执行用户；
3. 关键技巧：复杂/链式命令需用 `sh -c` 包裹，仅对运行中的容器有效，退出终端不停止容器。
...
### 一、命令核心作用
`docker container inspect`（可简写为 `docker inspect`）是 Docker 中用于**查询容器（或镜像、网络、卷等）详细元数据**的核心命令，核心特点：
1. 输出内容是结构化的 JSON 格式，包含容器的**所有配置和运行状态**（远多于 `docker ps` 展示的基础信息）；
2. 可查询单个或多个容器的信息，只需指定容器名/容器ID；
3. 支持通过参数过滤、格式化输出，精准获取需要的信息（比如容器IP、挂载卷、配置的环境变量等）；
4. 除了容器，该命令也适用于镜像、网络、卷等 Docker 资源（语法：`docker inspect 资源类型 资源名`）。

### 二、核心参数详解（Options）
| 参数 | 简写 | 作用 | 实用场景 | 关键说明 |
|------|------|------|----------|----------|
| `--format` | `-f` | 按自定义模板格式化输出 | 只提取需要的特定信息（如容器IP、挂载路径） | 支持 Go 模板语法，也可直接指定 `json` 输出完整JSON（默认就是JSON）；<br>模板语法参考 Docker 官方文档：https://docs.docker.com/go/formatting/ |
| `--size` | `-s` | 在输出中额外显示容器的**文件系统总大小** | 查看容器占用的磁盘空间 | 包含容器的可写层大小 + 只读层大小，默认不显示该信息 |

### 三、基础用法与典型场景示例
#### 场景1：查看单个容器的完整信息（默认用法）
这是最基础的用法，输出容器的全部JSON元数据：
```bash
# 通过容器名查询
docker container inspect mycontainer

# 通过容器ID查询（只需输入ID前几位，Docker会自动匹配）
docker container inspect 482efdf39fac
```
输出的JSON包含以下核心模块（新手重点关注）：
- `Id`：容器唯一ID；
- `Name`：容器名称；
- `State`：容器状态（Running/Paused/Stopped）、启动时间、PID等；
- `NetworkSettings`：网络配置（IP地址、端口映射、网关、所属网络等）；
- `Mounts`：挂载的卷/绑定目录（数据卷、宿主机绑定路径等）；
- `Config`：容器启动配置（环境变量、命令、工作目录、端口暴露等）；
- `HostConfig`：宿主机相关配置（资源限制、挂载配置、网络模式等）。

#### 场景2：提取特定信息（-f 参数，高频实用）
`-f` 参数是该命令最常用的功能，通过 Go 模板精准提取需要的信息，避免在海量JSON中翻找。

##### 示例1：获取容器的IP地址
```bash
docker container inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' mycontainer
# 输出示例：172.17.0.2
```
模板说明：`.NetworkSettings.Networks` 是容器的网络配置集合，`range` 遍历所有网络，`{{.IPAddress}}` 提取IP地址。

##### 示例2：获取容器的端口映射（比如容器80端口映射到宿主机的哪个端口）
```bash
docker container inspect -f '{{range $p, $conf := .NetworkSettings.Ports}}{{$p}} -> {{(index $conf 0).HostPort}}{{end}}' mycontainer
# 输出示例：80/tcp -> 8080
```
...
# 输出示例：[PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin VAR_A=1 VAR_B=2]
```

##### 示例4：获取容器的挂载卷/绑定目录
```bash
docker container inspect -f '{{.Mounts}}' mycontainer
# 输出示例：[{volume 8f9... /var/lib/docker/volumes/8f9.../_data /app/data local true }]
# 含义：卷类型、卷ID、宿主机路径、容器内路径、驱动、是否只读
```

##### 示例5：以JSON格式提取单个字段（便于程序解析）
```bash
# 输出格式化的JSON（而非纯文本）
docker container inspect -f '{{json .State}}' mycontainer
```

#### 场景3：查看容器的磁盘占用大小（-s 参数）
```bash
docker container inspect -s mycontainer
```
输出的JSON中会新增 `SizeRootFs`（容器只读层大小）和 `SizeRw`（容器可写层大小）字段，总和就是容器占用的总磁盘空间。

#### 场景4：查看多个容器的信息
只需在命令后追加多个容器名/ID即可：
```bash
docker container inspect mycontainer1 mycontainer2 482efdf39fac
```
输出是一个JSON数组，每个元素对应一个容器的信息。

### 四、实用模板速查表（直接复制使用）
为了方便你快速使用，整理了高频场景的模板：

| 需求 | 命令 |
|------|------|
| 查看容器状态（Running/Stopped） | `docker inspect -f '{{.State.Status}}' 容器名` |
| 查看容器启动命令 | `docker inspect -f '{{.Config.Cmd}}' 容器名` |
| 查看容器工作目录 | `docker inspect -f '{{.Config.WorkingDir}}' 容器名` |
| 查看容器所属网络名称 | `docker inspect -f '{{range .NetworkSettings.Networks}}{{.NetworkID}}{{end}}' 容器名` |
| 查看容器的宿主机绑定端口（所有） | `docker inspect -f '{{.NetworkSettings.Ports}}' 容器名` |

### 五、注意事项
1. **资源类型兼容**：`docker inspect` 不仅能查容器，还能查镜像（`docker inspect 镜像名`）、网络（`docker inspect 网络名`）、卷（`docker inspect 卷名`），语法完全一致；
2. **容器不存在时报错**：如果指定的容器名/ID不存在，会报错 `Error: No such object: 容器名`；
3. **模板语法容错**：如果提取的字段不存在（比如容器未配置端口映射），会输出空值，不会报错；
4. **JSON格式化输出**：如果觉得默认输出的JSON太紧凑，可结合 `jq` 工具美化（需先安装 `jq`）：
   ```bash
   docker inspect mycontainer | jq
   ```

### 总结
1. `docker container inspect` 是查询容器**全量元数据**的核心命令，输出为JSON格式，信息远多于 `docker ps`；
2. 核心参数：`-f` 按模板提取特定信息（高频实用）、`-s` 查看容器磁盘占用大小；
3. 关键技巧：记住常用的Go模板（如提取IP、端口、环境变量），可大幅提升排查问题的效率。


[目录](#目录)


## docker container start（别名 `docker start`）

### 一、命令核心作用
`docker container start` 是 Docker 中用于**启动一个或多个已经停止（stopped）的容器**的核心命令，核心特点：
1. 仅对**停止状态**的容器有效（容器状态为 `exited/stopped`，运行中/暂停的容器执行此命令会提示“容器已启动”）；
2. 启动容器时，会沿用容器**创建时的配置**（如端口映射、挂载卷、环境变量、启动命令等），无需重新指定；
3. 启动后的容器会恢复到停止前的运行逻辑（比如容器内的主进程会重新执行）；
4. 可批量启动多个容器，只需在命令后追加多个容器名/ID。

### 二、核心参数详解（Options）
| 参数 | 简写 | 作用 | 实用场景 | 关键说明 |
|------|------|------|----------|----------|
| `--attach` | `-a` | 附加到容器的标准输出（STDOUT）/标准错误（STDERR），并转发信号 | 启动容器后实时查看其输出日志 | 类似 `docker run -a`，启动后终端会阻塞，直到容器再次停止；按 `Ctrl+C` 仅断开附加，不会停止容器 |
| `--checkpoint` | - | 从指定的检查点恢复容器（实验性功能） | 容器快照恢复（极少用） | 需 Docker 守护进程开启实验性功能，生产环境几乎不用 |
| `--checkpoint-dir` | - | 指定检查点的存储目录（实验性功能） | 配合 `--checkpoint` 使用 | 同上，仅实验阶段，新手无需关注 |
| `--detach-keys` | - | 自定义“脱离容器”的快捷键 | 替换默认的 `Ctrl+P,Ctrl+Q` 脱离组合键 | 新手极少用到，主要避免快捷键冲突 |
| `--interactive` | `-i` | 附加容器的标准输入（STDIN） | 启动容器后需要和容器内进程交互 | 通常和 `-a` 组合使用（`-ai`），模拟交互式启动；单独 `-i` 无实际意义，需配合 `-a` 才能交互 |

> 注：标有 **experimental (daemon)** 的参数是实验性功能，未正式发布，日常使用中几乎不会接触，重点关注 `-a` 和 `-i` 即可。

### 三、典型使用场景与示例
#### 场景1：基础用法 - 启动单个停止的容器（最常用）
这是 `docker start` 最核心的用法，无参数启动容器，容器在后台运行：
```bash
# 通过容器名启动
docker start my_container

# 通过容器ID启动（只需输入前几位，Docker自动匹配）
docker start 482efdf39fac
```
执行后会输出容器名/ID，表示启动成功；可通过 `docker ps` 验证容器状态变为 `running`。

#### 场景2：启动容器并实时查看其输出（-a 参数）
启动容器后立即附加到其输出流，查看容器内主进程的日志：
```bash
# 启动容器并附加到输出，实时查看日志
docker start -a my_container

# 示例输出（假设容器内主进程是打印日志）：
# 2026-02-28 10:00:00 - App started
# 2026-02-28 10:00:01 - Listening on port 8080
```
- 此模式下终端会阻塞，直到容器停止（比如主进程退出）；
- 若想断开附加但保留容器运行，按 `Ctrl+C` 即可（区别于 `docker run -it`，这里不会停止容器）。

#### 场景3：启动容器并保持交互式输入（-ai 参数）
启动容器后保持标准输入/输出打开，支持和容器内进程交互（比如容器主进程是 `sh`/`bash`）：
```bash
# 启动容器并进入交互式模式
docker start -ai my_container

# 执行后会进入容器的shell，可输入命令（如 ls、pwd 等）
# 退出交互：执行 exit（容器会再次停止）或按 Ctrl+P,Ctrl+Q（容器保持运行，仅退出交互）
```

#### 场景4：批量启动多个停止的容器
只需在命令后追加多个容器名/ID，即可批量启动：
```bash
# 启动 my_container1、my_container2 两个容器
docker start my_container1 my_container2

# 启动容器ID为 482efdf39fac 和 9f87654321ab 的容器
docker start 482efdf39fac 9f87654321ab
```

### 四、关键对比（避免和其他命令混淆）
| 命令 | 核心区别 |
|------|----------|
| `docker start` | 启动**已存在的停止容器**，沿用原有配置，无新容器创建 |
| `docker run` | 创建**新容器**并启动，每次执行都会生成新容器（除非指定 `--rm`） |
| `docker restart` | 重启**运行中的容器**（先停止，再启动），等同于 `docker stop + docker start` |
| `docker unpause` | 恢复**暂停（paused）** 的容器，容器进程未停止，只是暂停执行，启动速度远快于 `docker start` |

### 五、常见问题与注意事项
1. **容器启动失败**：
    - 原因1：容器创建时的主进程配置错误（比如命令不存在）；
    - 原因2：容器依赖的资源（如挂载的宿主机目录被删除、网络被移除）；
    - 排查：执行 `docker logs 容器名` 查看启动失败的日志。

2. **端口冲突**：
    - 若容器启动时绑定的端口已被宿主机其他进程占用，启动会失败；
    - 解决：停止占用端口的进程，或修改容器端口映射（需先删除容器，重新创建）。

3. **启动后容器立即停止**：
    - 原因：容器内的主进程执行完立即退出（比如 `docker run` 时指定的命令是 `ls`，执行完就结束）；
    - 解决：确保容器主进程是“常驻进程”（如 `sh`、`nginx -g 'daemon off;'`）。

### 总结
1. `docker container start`（简写 `docker start`）用于启动**停止状态**的容器，沿用容器创建时的所有配置；
2. 核心参数：`-a` 附加到容器输出、`-i` 保持标准输入，常用组合 `--ai` 实现交互式启动；
3. 关键区别：`start` 启动已有容器，`run` 创建新容器，`restart` 重启运行中容器，`unpause` 恢复暂停容器。

[目录](#目录)

## docker container stop（别名 `docker stop`）

### 一、命令核心作用
`docker container stop` 是 Docker 中用于**停止一个或多个运行中的容器**的核心命令，其底层逻辑是“优雅关停”而非“强制杀死”，核心特点：
1. 对**运行中（running）** 的容器生效（暂停/停止的容器执行此命令会提示“容器已停止”）；
2. 关停流程分两步（默认）：
    - 第一步：向容器内的主进程（PID 1）发送 `SIGTERM` 信号，给容器“优雅退出”的时间（比如保存数据、释放资源、关闭连接）；
    - 第二步：等待**超时时间**（默认Linux 10秒/Windows 30秒）后，若容器仍未停止，发送 `SIGKILL` 信号强制杀死进程，容器立即停止；
3. 可批量停止多个容器，也可自定义发送的信号和超时时间；
4. 停止后的容器状态变为 `exited/stopped`，可通过 `docker start` 重新启动，且数据不会丢失（容器内的可写层数据保留）。

### 二、核心参数详解（Options）
| 参数 | 简写 | 默认值 | 作用 | 实用场景 | 关键说明 |
|------|------|--------|------|----------|----------|
| `--signal` | `-s` | `SIGTERM`（或镜像/容器配置的 `StopSignal`） | 自定义发送给容器主进程的信号 | 需要非默认信号关停容器（如 `SIGINT` 模拟Ctrl+C、`SIGUSR1` 触发自定义退出逻辑） | 支持两种格式：<br>1. 信号名：`SIGKILL`、`SIGINT`（可省略 `SIG`，直接写 `KILL`/`INT`）；<br>2. 信号数字：`9`（对应 `SIGKILL`）、`15`（对应 `SIGTERM`）；<br>信号列表参考：[signal(7)](https://man7.org/linux/man-pages/man7/signal.7.html) |
| `--timeout` | `-t` | Linux:10秒 / Windows:30秒 | 设置等待容器优雅退出的超时时间 | 给需要更长时间清理的容器（如数据库、大数据服务）设置更长超时 | 单位：秒；<br>- 设置 `-1` 表示无超时，无限等待容器自行退出；<br>- 超时后自动发送 `SIGKILL` 强制停止 |

### 三、典型使用场景与示例
#### 场景1：基础用法 - 停止单个容器（最常用）
无参数停止容器，使用默认的 `SIGTERM` 信号和超时时间：
```bash
# 通过容器名停止
docker stop my_container

# 通过容器ID停止（只需输入前几位）
docker stop 482efdf39fac
```
执行流程：
1. Docker 向容器主进程发送 `SIGTERM`（15）信号；
2. 等待10秒（Linux），若容器未停止，发送 `SIGKILL`（9）信号强制停止；
3. 输出容器名/ID，表示停止成功，可通过 `docker ps -a` 验证状态为 `exited`。

#### 场景2：自定义超时时间停止容器（-t 参数）
给需要长时间清理的容器（如MySQL、Elasticsearch）设置更长的超时时间：
```bash
# 给容器30秒优雅退出时间，超时后强制停止
docker stop -t 30 mysql_container
```
适用场景：数据库容器停止时需要刷写内存数据到磁盘，10秒可能不够，需延长超时时间避免数据丢失。

#### 场景3：自定义信号停止容器（-s 参数）
##### 示例1：直接强制停止容器（跳过优雅退出，等效 `docker kill`）
```bash
# 发送 SIGKILL（9）信号，立即强制停止容器，无超时等待
...
```
⚠️ 注意：`SIGKILL` 会直接杀死进程，容器内程序没有时间保存数据，可能导致数据损坏，仅在容器卡死时使用。

##### 示例2：发送 `SIGINT` 信号（模拟Ctrl+C）
```bash
# 发送 SIGINT（2）信号，适合需要响应Ctrl+C退出的程序（如Python脚本、Node.js服务）
docker stop -s SIGINT app_container
```

#### 场景4：批量停止多个容器
只需在命令后追加多个容器名/ID，即可批量停止：
```bash
# 停止 my_container1、my_container2 两个容器
docker stop my_container1 my_container2

# 停止容器ID为 482efdf39fac 和 9f87654321ab 的容器
docker stop 482efdf39fac 9f87654321ab
```

### 四、关键原理与扩展说明
#### 1. `StopSignal` 的优先级（自定义默认停止信号）
容器接收的默认停止信号并非固定为 `SIGTERM`，优先级从高到低：
1. `docker stop -s` 手动指定的信号；
2. 容器创建时 `docker run --stop-signal` 指定的信号；
3. 镜像构建时 `Dockerfile` 中 `STOPSIGNAL` 指令设置的信号；
4. Docker 守护进程默认信号（`SIGTERM`）。

示例（Dockerfile 设置 `STOPSIGNAL`）：
```dockerfile
# 构建镜像时指定默认停止信号为 SIGINT
FROM alpine
STOPSIGNAL SIGINT
CMD ["sh", "-c", "trap 'echo Exiting...; exit 0' INT; while true; do sleep 1; done"]
```
构建后启动容器，执行 `docker stop 容器名` 时，默认发送的是 `SIGINT` 而非 `SIGTERM`。

#### 2. 与 `docker kill`/`docker pause` 的核心区别
| 命令 | 核心行为 | 适用场景 | 数据风险 |
|------|----------|----------|----------|
| `docker stop` | 先 `SIGTERM` 优雅退出，超时后 `SIGKILL` | 常规关停容器（推荐） | 低（有数据保存时间） |
| `docker kill` | 直接发送 `SIGKILL` 强制停止 | 容器卡死、无响应时紧急停止 | 高（可能数据损坏） |
| `docker pause` | 暂停容器进程（发送 `SIGSTOP`），不终止进程 | 临时暂停容器（如备份、调试） | 无（恢复后进程继续运行） |

### 五、常见问题与注意事项
1. **容器停止后立即重启**：
    - 原因：容器创建时设置了 `--restart` 策略（如 `always`/`unless-stopped`）；
    - 解决：先执行 `docker update --restart=no 容器名` 取消重启策略，再停止容器。

2. **容器超时后仍未停止**：
    - 原因：容器主进程忽略 `SIGTERM` 信号，或卡死无法响应；
    - 解决：执行 `docker stop -s 9 容器名` 强制停止，或排查容器内进程卡死原因（通过 `docker logs`/`docker top`）。

3. **停止容器后数据丢失**：
    - 原因：数据未挂载到卷/宿主机目录，容器可写层被删除（仅 `docker rm` 会删，`docker stop` 不会）；
    - 解决：确保重要数据通过 `-v` 挂载到卷或宿主机目录。

### 总结
1. `docker container stop`（简写 `docker stop`）是“优雅关停”运行中容器的核心命令，默认先发 `SIGTERM` 给容器留退出时间，超时后发 `SIGKILL` 强制停止；
2. 核心参数：`-t` 自定义超时时间（给慢服务留足够清理时间）、`-s` 自定义停止信号（如 `9` 强制停止）；
3. 关键区别：`stop` 优雅关停（推荐），`kill` 强制杀死（应急用），`pause` 临时暂停（不终止进程）。
